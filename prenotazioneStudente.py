...
#VARIABILE GLOBALE CON ID, SCADENZA ISCRIZIONE IN MENÙ??
idstud = 1
CFistruttore = get_istruttore_pratico_studente(idstud)[0][0]
...
def updateLabel():
    result = get_guide_rimaste(idstud)
    if result:
        num = result[0][0]
    else:
        num = 0
...
def addGuida():
    ...
    idPacchetto = get_id_pacchetto(idstud)[0][0]
    if not data or not ora or not targa or not CFistruttore or not idPacchetto:
        ...
        if data_obj < datetime.datetime.now().date():
            ...
            if get_guide_rimaste(idstud)[0][0] == 0:
